...
class Tagger_Evidence(nn.Module):
    ...
    def edl_loss(self, logits, sequence_labels, input_mask, epoch):
        ...
        sequence_labels = sequence_labels.cuda()
        input_mask = input_mask[:,:self.seqlen]
        input_mask = input_mask.cuda()

        soft_output = get_tagger_one_hot(sequence_labels, self.num_classes, 0, 1, input_mask, self.gpu) # one-hot label
        
        log_probs = torch.nn.functional.log_softmax(logits, dim=2)
        cross_entropy_loss = -torch.sum(soft_output * log_probs, dim=2)
        masked_loss = torch.masked_select(cross_entropy_loss, input_mask.bool())
...
#         loss= loss.mean()
        ...
        # return loss, alpha/S, alpha, uncertainty
        return loss, bu, alpha, uncertainty
