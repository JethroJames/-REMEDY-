...
class MyModel(nn.Module):
    ...
    def _bert_forward_with_image(self, inputs, pairs, gate_signal=None):
        ...
        # visual_embeds: [bs, chanels, H, W]  RGB:channels = 3, H = W = 224
    ...
    def _bert_forward_with_caption(self, inputs, pairs, gate_signal=None):
        # query
        textual_embeds = self.encoder_t.embeddings.word_embeddings(inputs.input_ids)
        # caption
        ...
        if gate_signal is not None:
            textual_embeds_captions *= gate_signal
        # query + caption
        inputs_embeds = torch.cat((textual_embeds, textual_embeds_captions), dim=1)

        batch_size = textual_embeds_captions.size()[0]
        caption_length = textual_embeds_captions.size()[1]
    ...
    # gpt1
    def _bert_forward_with_gpt1(self, inputs, pairs, gate_signal=None):
    ...
    # gpt2
    def _bert_forward_with_gpt2(self, inputs, pairs, gate_signal=None):
    ...
    def ner_encode(self, pairs: List[MyPair], gate_signal=None):
        ...
        # T + I view
        INT = self._bert_forward_with_image(inputs, pairs, gate_signal)
        INT_feat_batch = INT.last_hidden_state[:, :-VISUAL_LENGTH]
        
        # T + C view 
        INC,caption_length = self._bert_forward_with_caption(inputs, pairs, gate_signal)
        INC_feat_batch = INC.last_hidden_state[:, :-caption_length]

        # T + C + G1 view
        ICG1, ICG1_length = self._bert_forward_with_gpt1(inputs, pairs, gate_signal)
        ICG1_feat_batch = ICG1.last_hidden_state[:, :-ICG1_length]

        # T + C + G2 view
        ICG2, ICG2_length = self._bert_forward_with_gpt2(inputs, pairs, gate_signal)
