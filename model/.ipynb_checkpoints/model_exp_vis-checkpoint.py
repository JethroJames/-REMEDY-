...
# 
    ...
    def ner_forward(self, pairs: List[MyPair], epoch):
        ...
        if self.edl:
            ...
            # Student
            edl_stu_res = {}
            edl_loss_stu, uncertainty, evidence_stu = self.edl_model.loss(stu_logits_batch, labels_batch, mask_batch, epoch)
            edl_loss += edl_loss_stu
            
            edl_stu_res[0] = {
                'edl_loss': edl_loss_stu,
                'uncertainty': uncertainty,
                'evidence': evidence_stu 
            }
            evidences[0] = evidence_stu
            # Teacher
            edl_tea_res = {}

            for view, tea_logits_bc in tea_logits_batch.items():
                # 对每个视图使用EDL模型计算损失、不确定性和证据向量
                edl_loss_tea, uncertainty, evidence = self.edl_model.loss(tea_logits_bc, labels_batch, mask_batch, epoch)
            ...
            # 学生和教师的不确定性与证据向量已通过之前的步骤计算出来，存储在edl_stu_res和edl_tea_res中

            # 获取学生模型的不确定性和证据向量
            student_uncertainty = edl_stu_res[0]['uncertainty'].cpu().detach().numpy()
            student_evidence = edl_stu_res[0]['evidence'].cpu().detach().numpy()

            # 初始化总的加权证据向量和总的权重
            total_weighted_evidence = np.zeros_like(student_evidence)
            total_weights = 0

            # 学生证据的加权因子（例如，可以使用不确定性的倒数作为权重）
            student_weight = 1 / (student_uncertainty + 1e-6)
            total_weighted_evidence += student_weight * student_evidence
            total_weights += student_weight

            # 教师证据的加权
            for view, res in edl_tea_res.items():
                teacher_uncertainty = res['uncertainty'].cpu().detach().numpy()
                teacher_evidence = res['evidence'].cpu().detach().numpy()
                # 教师证据的加权因子
                teacher_weight = 1 / (teacher_uncertainty + 1e-6)
                total_weighted_evidence += teacher_weight * teacher_evidence
                total_weights += teacher_weight

            # 归一化加权求和的证据向量
            evidence_a = total_weighted_evidence / total_weights  # 此时的evidence_a是根据不确定性加权的学生和教师证据的合并结果

            # evidence_a = evidence_stu
            # for i in range(1, len(edl_tea_res)):
            #     evidence_a = (evidences[i] + evidence_a) / 2
            evidence_a = torch.from_numpy(evidence_a).to(self.device)

            edl_loss_A, uncertainty_a, evidence_A = self.edl_model.loss(evidence_a, labels_batch, mask_batch, epoch)
            edl_loss += edl_loss_A
            edl_loss = edl_loss / (len(evidences) + 1) # fusion view & tea-stu views
        ...
        loss = edl_loss + crf_loss + rcml_loss
        return loss, pred
