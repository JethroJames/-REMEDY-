...
def get_dc_seq_loss(evidences_dict, device):
    num_views = len(evidences_dict)
    # 取第一个元素来获取 batch_size, seq_length, 和 class_num
    batch_size, seq_length, num_classes = next(iter(evidences_dict.values())).shape
    p = torch.zeros((num_views, batch_size, seq_length, num_classes)).to(device)
    u = torch.zeros((num_views, batch_size, seq_length)).to(device)
    
    for v, evidence in evidences_dict.items():
        alpha = evidence + 1
        S = torch.sum(alpha, dim=2, keepdim=True)  # Sum across classes
        # pdb.set_trace()
        p[v] = alpha / S  # Probability distribution
        u[v] = torch.squeeze(num_classes / S, dim=2)  # Uncertainty
    
    dc_sum = 0
    for i in range(num_views):
        pd = torch.sum(torch.abs(p - p[i]) / 2, dim=3) / (num_views - 1)  # Pairwise distance
        cc = (1 - u[i].unsqueeze(0)) * (1 - u)  # Pairwise complementary confidence
        dc = pd * cc  # Disagreement cost
        dc_sum += torch.sum(dc, dim=(0, 2))  # Sum over views and seq_length
    
    dc_sum = torch.mean(dc_sum)  # Mean over batch_size
    return dc_sum