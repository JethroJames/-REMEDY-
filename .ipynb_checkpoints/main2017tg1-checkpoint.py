...
from model.modeltg1 import MyModel
...
model_weights_path = f'model_weights/{current_time}_{args.dataset}/'
model_log_path = f'log/{current_time}_{args.dataset}/'
...
for epoch in range(1, args.num_epochs + 1):
    ner_loss = train(ner_train_loader, model, optimizer,epoch, task='ner', weight=1.0)
    ...
    # 保存模型权重
    model_save_name = f'model_weights/{current_time}_{args.dataset}/model_bs{args.bs}_lr{args.lr}_seed{args.seed}.pt'
...
# 修改文件名以包含时间戳
file_name = f'log/{current_time}_{args.dataset}/bs{args.bs}_lr{args.lr}_seed{args.seed}.json'
