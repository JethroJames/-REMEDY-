...
from data import loadersnap
from model.model_uncertainty_fusion_rcml import MyModel
...
parser.add_argument('--dataset', type=str, default='snapC', choices=['twitter2015', 'twitter2017','snapC'])
...
model_weights_path = f'model_weights/snap_u-rcml/{current_time}_{args.dataset}/'
model_log_path = f'log/snap_u-rcml/{current_time}_{args.dataset}/'
...
'''
    Load Dataset
'''
ner_corpus = loadersnap.load_ner_corpus(f'resources/datasets/{args.dataset}', load_image=(args.encoder_v != ''))
...
for epoch in range(1, args.num_epochs + 1):
    ...
    # 保存模型权重
    model_save_name = f'model_weights/snap_u-rcml/{current_time}_{args.dataset}/model_bs{args.bs}_lr{args.lr}_seed{args.seed}.pt'
    torch.save(model.state_dict(), model_save_name)
    print(f'Model weights saved to {model_save_name}')

    # 保存不确定性数据
    with open(f'model_weights/snap_u-rcml/data_epoch{epoch}.pkl','wb') as f:
...
# 修改文件名以包含时间戳
file_name = f'log/snap_u-rcml/{current_time}_{args.dataset}/bs{args.bs}_lr{args.lr}_seed{args.seed}.json'
