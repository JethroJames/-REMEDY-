...
from model.model_uncertainty_fusion_lamda import MyModel
from utils_uncertainty_fusion import seed_worker, seed_everything, train, evaluate
import pdb
import netron
import torch.onnx
...
parser.add_argument('--lamda1', type=float, default=1.0, help='CRF 损失的权重乘数')
parser.add_argument('--lamda2', type=float, default=1.0, help='RCML 损失的权重乘数')
...
model_weights_path = f'model_weights/2015_uncertainty_fusion_lamda/{current_time}_{args.dataset}/'
model_log_path = f'log/2015_uncertainty_fusion_lamda/{current_time}_{args.dataset}/'
...
for epoch in range(1, args.num_epochs + 1):
    ...
    # 保存模型权重
    model_save_name = f'model_weights/2017_uncertainty_fusion_lamda/{current_time}_{args.dataset}/model_bs{args.bs}_lr{args.lr}_seed{args.seed}.pt'
    torch.save(model.state_dict(), model_save_name)
    print(f'Model weights saved to {model_save_name}')

    # 保存不确定性数据
    with open(f'model_weights/2017_uncertainty_fusion_lamda/data_epoch{epoch}.pkl','wb') as f:
...
# 修改文件名以包含时间戳
file_name = f'log/2017_uncertainty_fusion_lamda/{current_time}_{args.dataset}/bs{args.bs}_lr{args.lr}_seed{args.seed}.json'
