...
from model.model_uncertainty_fusion_tic import MyModel
...
model_weights_path = f'model_weights/2017_uncertainty_fusion-tic/{current_time}_{args.dataset}/'
model_log_path = f'log/2017_uncertainty_fusion-tic/{current_time}_{args.dataset}/'
...
for epoch in range(1, args.num_epochs + 1):
    ...
    if dev_f1 > best_dev_f1:
        ...
        # 保存模型权重
        model_save_name = f'model_weights/2017_uncertainty_fusion-tic/{current_time}_{args.dataset}/model_bs{args.bs}_lr{args.lr}_seed{args.seed}_epoch{epoch}.pt'
        torch.save(model.state_dict(), model_save_name)
        print(f'Model weights saved to {model_save_name}')

    # 保存不确定性数据
    with open(f'model_weights/2017_uncertainty_fusion-tic/data_epoch{epoch}.pkl','wb') as f:
...
# 修改文件名以包含时间戳
file_name = f'log/2017_uncertainty_fusion-tic/{current_time}_{args.dataset}/bs{args.bs}_lr{args.lr}_seed{args.seed}.json'
