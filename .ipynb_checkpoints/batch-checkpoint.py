...
sentence = []
optimizer = getattr(torch.optim, args.optim)(params)
# ner_loss, uncertainty, batch_data = train(ner_train_loader, model, optimizer,epoch, task='ner', weight=1.0)
batch_data_list = []  # 最外层列表，包含所有批次的数据
model = MyModel.from_pretrained(args)
model_path = 'model_weights/2015_uncertainty_fusion-t/2024-04-18_10-09-39_twitter2015/model_bs4_lr1e-05_seed0_epoch3.pt'
model.load_state_dict(torch.load(model_path))
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model.to(device)

for epoch in range(1, args.num_epochs + 1):
    batch_data_list,sentence = train(ner_train_loader, model, optimizer,epoch, task='ner', weight=1.0)
    # with open('batch_data2015{epoch}.pkl', 'wb') as f:
    #     pickle.dump(batch_data_list, f)
    with open(f'sentence2015{epoch}.pkl', 'wb') as f:
        pickle.dump(batch_data_list, f)

# for batch in tqdm(ner_train_loader):
...
# # 现在batch_data_list是一个包含嵌套列表的列表，可以被pickle序列化
